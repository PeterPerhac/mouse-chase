...
case class Vector2(var x: Double, var y: Double) {
  ...
  def normalized: Vector2 = this / magnitude
...
object Implicits {
  ...
  implicit def pointToPoint2D(p: Point): Vector2 = Vector2(p.x, p.y)

  implicit def point2DToPoint(p: Vector2): Point = Point(p.x, p.y)
}
